...
function _mapAvatar(avatar, fallbackPhotoUrl = '') {
  // numeric columns can come back as strings; anything unparseable is "not set"
  const num = v => (Number.isFinite(Number(v)) && v !== null && v !== '' ? Number(v) : null);
  return {
    name: avatar.name || 'Me',
    photoUrl: avatar.signedPhotoUrl || fallbackPhotoUrl,
    heightCm: num(avatar.height_cm),
    weightKg: num(avatar.weight_kg),
...
async function saveAvatarRemote(name, photoDataUrl, { heightCm, weightKg } = {}) {
...
async function generateAvatarCandidates({ photos, name, heightCm, weightKg, signal }) {
  const res = await _edge('avatar', {
    method: 'POST',
    signal,
    body: JSON.stringify({
...
async function commitAvatarCandidate({ candidatePath, name, heightCm, weightKg }) {
  const res = await _edge('avatar', {
    method: 'POST',
    body: JSON.stringify({
      action: 'commit',
